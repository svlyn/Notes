...
-   `<base>`, specifies the base URL to use for all relative  contained within a document. **There can be only one <base> element in a document.**
    The base URL of a document can be queried from a script using _`document.baseURI.`_

    -   Eg.: if the image is `<img src="http://example.com/i/pic.gif" />` Then we could set the `<base>`tag in `<head>`.
...
-   `<meta>`, help to define the webpage.

    -   Eg: define keywords, author, released date. Add a description or a redirect link to the page.
...
-   `<main>` Use the main element between header and footer elements to contain the primary content of your web page.
    ...
    -   OK to have more than one `main` element in a webpage.
        ...
        <h1>TechOnTheNet.com</h1>
          ...
          <article>
            <h2>HTML</h2>
            <p>Learn HTML (HyperText Markup Language) with our step-by-step utorials and references.</p>
          </article>
          <article>
            <h2>CSS</h2>
